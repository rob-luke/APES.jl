...
#=f = 0.0:0.001:0.5=#
#=a = abs(apes(sn, f))=#
#=a = layer(x=f, y=a, Geom.line,Theme(default_color=color("green")))=#
#=f = layer(x=freq(periodogram(sn, nfft=N*16)), y=sqrt(power(periodogram(sn, nfft=N*16))/(length(sn)/2)), Geom.line,Theme(default_color=color("red")))=#
#=p = plot(a, f, Guide.title("FFT (red) APES(green)"), Guide.xlabel("Frequency"), Guide.ylabel("Amplitude"))=#
#=draw(PNG("Eg.png", 28cm, 14cm), p)=#
